function [xbest, fbest] = miso2(datafile,maxeval, surrogate, n_start, init_design, sampling, ...
    own_design,Data)
...
% filename=strcat('loadfiles\100d\',num2str(0),'_',num2str(Q),'.mat');
%   load (filename);

% S=Data.S;   
%load optimization problem data from user-defined file
% Data=feval(datafile); % load problem data
Data.objfunction=@(x)datainput_dp2(x);
% Data.S=S;
...
%check if the type of initial experimental design is defined   %³õÊ¼ÊµÑéÉè¼Æ×Ü¹²ÓÐËÄÖÖ
fixrank = false;
...
elseif strcmp(init_design, 'own') %user has to supply a matrix with selected sample points to be included in initial design
    ...
    if size(own_design,2) < Data.dim %check if given points are of the correct dimension      %Ð£ÑéÎ¬¶È
        error('The matrix with initial design points has the wrong dimension (number of columns must equal problem dimension)!')
    elseif (size(own_design,1) < Data.dim+1 && isempty(n_start)) || (~isempty(n_start) && n_start +size(own_design,1) < Data.dim+1)     %Ð£ÑéÐÐÊý
        disp('User given initial design does not have sufficiently many points to fit RBF. Filling in remaining points by slhd!')
        n_start = Data.dim + 1 - size(own_design,1);      %ÕâÀï n_start±ä³É´æ·ÅÐèÒªÌí¼ÓµÄÐÐÊý
    else %user given design contains sufficiently many points   %ÐÐºÍÁÐ¶¼Âú×ãÁË
        %check rank of matrix
        if rank([own_design, ones(size(own_design,1),1)]) < Data.dim + 1     %ÈôÖÈ<n+1
            disp('Rank of user given initial design matrix is too low. Generating more points to satisfy rank condition')
            n_start = Data.dim-rank(own_design);       %ÕâÀïÁî n_start=n-r
        else
            n_start = 0;
        end
    end
...
Data.tol = 0.001*min(Data.xup-Data.xlow); %tolerance when 2 points are considered the same Á½¸öµã½Ó½ü³Ì¶ÈµÄÈÝÈÌ¶È
...
% end
% %round integer variables to integers............................ËÄÉáÎåÈë£¡
% Data.S(:,Data.integer)=round(Data.S(:,Data.integer));   %ËùÓÐÐÐµÄÕûÊýÎ¬¶È.ÁÐ
