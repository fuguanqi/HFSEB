...
function y=datainput_cplex2(x) %objective function
xx=x(:)'; % make sure vector is row vector
load (strcat('problems\Data2.mat'));
...
for i=1:n
    for j=1:g
        for v=1:l
            for i1=1:n
                Constraints=[Constraints, ...
                    C_minus(i,j)+(2-W(i1,i,j,K(i,j))-V(i,j,v))*M>=C_minus(i1,j)+t_P(i,j,K(i,j),v), ...
                    ];
            end
            Constraints=[Constraints, ...
                C_minus(i,j)+(1-V(i,j,v))*M>=A(i,j)+t_P(i,j,K(i,j),v), ...
                ];
        end
        Constraints=[Constraints, ...
            U(i,j,K(i,j))==1, ...
